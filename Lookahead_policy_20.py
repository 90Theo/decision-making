...
def create_professor_model(price_arr, occ_r1_arr, occ_r2_arr, data):
    ...
    def vent_uptime2(m):
        ...
        if remaining <= 0 or remaining >= data['vent_min_up_time']: return pyo.Constraint.Skip
        end_t = min(remaining, num_t)
        return sum(m.v[tau] for tau in range(0, end_t)) >= end_t
